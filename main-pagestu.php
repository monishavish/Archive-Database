...
<body>
	<img src="http://localhost/project/banner.jpg" class="image">
...
<div style="overflow:auto;width:55%;height:20em;background:#eee;float:left;position:absolute;left:25%;top:60%;padding:3px;text-align: center;">
  <?php 
            ...
            $sql1= "SELECT stu_name,achievement,dept_name from stu_achieve f ,department d where f.dept_id=d.dept_id";
...
<h2 style="position:absolute;top:135%;left:25%;font-family: 'Muli', sans-serif;"><b>Academic positions.</b></h2>
...
<?php
    require_once('db_connection.php');
            $conn=OpenCon();
            $sql="SELECT top10name,dept_name,position,marks FROM academics a,department d WHERE a.dept_id=1 and d.dept_id=1 ORDER BY position" ;
    ...
    require_once('db_connection.php');
            $conn=OpenCon();
            $sql="SELECT top10name,dept_name,position,marks FROM academics a,department d WHERE a.dept_id=2 and d.dept_id=2 ORDER BY position" ;
            $result1= mysqli_query($conn,$sql);
            while($row1 = mysqli_fetch_assoc($result1)){
